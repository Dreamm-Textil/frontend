...
// Перевірити, чи є значення пустим
      ...
      if (!displayedElements.includes(json.id)) {
        ...
        cartProductsPage.innerHTML += `<div class="products-container">
                                        ...
                                        <table class="cart__config">
                                          <tbody><tr>
                                              ...
                                              <td>${colorMap.get(json.color)}</td>
                                          </tr>
                                          </tr>
                                              <th class="material-cart">Матеріал:</th>
                                              <td>${materialMap.get(json.material)}</td>
                                          </tr>
                                          </tbody>
                                        </table>
                                        <span type="number" class="cart-price" data-id="${json.id}" value="1">${json.discountPrice} грн</span>
                                        <div class="cart__counter">
                                          <span>Кількість:</span>
                                          <input  name="quantity" сlass="input-quantity" data-id="${json.id}" value="${quantity}">
                                          <div class="arrow-container">
                                            <img type="button" src="images/increase.png" class="increase" data-id="${json.id}"></img>
                                            <img class="decrease" type="button" src="images/increase.png" data-id="${json.id}"></img>
                                          </div>
                                        </div>
                                        <span class="cart-price-all" data-id="${json.id}" value="0" id="cart-price-all-${json.id}">${json.discountPrice * quantity} грн</span>
                                        <button data-id="${json.id}" class="cart-delete-btn"><img src="images/delete.png"></button>
                                      </div> `;
        ...
        let cartPriceAllElements = document.querySelectorAll(".cart-price-all");
        ...
        const cartSummaryTitle = document.querySelector('.cart-summaty-title');
        ...
        deleteBtn.forEach((button)=>{
          button.addEventListener('click', () => {
            ...
            const id = container.querySelector('.cart-price-all').getAttribute('data-id');
            ...
            const storedQuantity = localStorage.getItem('numberLS');
            const storedQuantityArray = storedQuantity ? storedQuantity.split(',') : [];
            const updatedQuantityArray = storedQuantityArray.filter((itemId) => itemId !== id);
...
// --------------------------------------------
        ...
        const quantityInputse = document.querySelectorAll('input[name="quantity"]');

        // Add event listeners to each quantity input
        quantityInputse.forEach(quantityInput => {
        ...
        function updateCartPrice(id, quantity) {
          ...
          const cartPriceElement = document.querySelector(`.cart-price[data-id="${id}"]`);
          const cartPriceAllElement = document.querySelector(`.cart-price-all[data-id="${id}"]`);
